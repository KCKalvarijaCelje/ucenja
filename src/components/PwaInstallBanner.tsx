...
const STORAGE_KEY_INSTALLED = 'kck_ucenja_pwa_installed';
const STORAGE_KEY_DISMISSED = 'kck_ucenja_pwa_dismissed_until';
...
export const PwaInstallBanner: React.FC<PwaInstallBannerProps> = ({ currentLang }) => {
  ...
  useEffect(() => {
    ...
    // Show banner after brief delay if not dismissed and not installed
  ...
  const handleInstallClick = async () => {
    if (deferredPrompt) {
      ...
      // Fallback instruction for iOS and mobile browsers without deferred prompt
      alert(
        currentLang === 'sl'
  ...
  return (
    <div className="md:hidden fixed bottom-20 sm:bottom-6 right-4 sm:right-6 z-40 max-w-sm w-full bg-[#064E3B] text-white p-4 rounded-2xl shadow-2xl border border-emerald-500/40 animate-in slide-in-from-bottom-6 duration-300">
