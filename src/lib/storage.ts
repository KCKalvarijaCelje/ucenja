...
export type MediaFolder = "sermons" | "teachers" | "series" | "uploads";

export async function uploadMedia(
  file: File | Blob,
  folder: MediaFolder = "sermons",
  ...
  if (supabase) {
    try {
      await supabase.storage.from(BUCKET).upload(path, fileToUpload, {
        cacheControl: "3600",
        upsert: false,
        contentType,
      });
    } catch (sbErr) {
      console.warn("Supabase storage backup warning:", sbErr);
    }
  }
